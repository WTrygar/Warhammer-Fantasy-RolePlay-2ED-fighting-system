...
class Character(ABC): #ABC prevents using this class, we are using only children
  ...
  def attack(self, target, is_counter: bool = False) -> None:
    #skip attack if own health reached zero
    if not self.alive:
  ...
  def get_damaged(self, dmg: int, attacker) -> None:
    ...
    print(f"{attacker.name} dealt {dmg} damage to {self.name} with {attacker.weapon.name}")

#subclass setup 
class Hero(Character):
  def __init__(self, name: str, health: int) -> None:
    super().__init__(name=name, health=health, eva_ch=10, crit_ch=10, armor=2)
